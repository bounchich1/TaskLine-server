...
function form(raw: string): Map<string, string> {
  ...
  for (const part of raw.split('&')) {
    ...
    let value: string;
    try {
      key = decodeURIComponent(part.slice(0, index).replaceAll('+', ' '));
      value = decodeURIComponent(part.slice(index + 1).replaceAll('+', ' '));
    } catch {
...
export function verifyLaunch(
  ...
  botToken: string,
  now = Date.now(),
): { userId: string; digest: string; startParam?: string } {
  ensure(
    Buffer.byteLength(raw) <= 16384 && raw.length > 0 && botToken.length > 0,
    'invalid_launch',
    401,
  );
  ...
  const date = Number(dateText) * 1000;
    ...
    now - date <= 300000 && date - now <= 30000,
    'expired_launch',
    401,
...
export async function issueSession(db: Database, c: Config, userId: string, launchHash: string) {
  return db.tx(async (tx) => {
    ...
    const recent = await one(
      tx,
      "SELECT count(*)::int AS n FROM staff_sessions WHERE launch_hash=$1 AND issued_at>now()-interval '5 minutes'",
      [launchHash],
    );
    ensure(
      Number(recent?.n) < 5,
    ...
    const csrf = token();
    await tx.query(
      'INSERT INTO staff_sessions(hash,org_id,employee_id,employee_version,csrf_hash,launch_hash) VALUES($1,$2,$3,$4,$5,$6)',
      [hash(secret), c.ORG_ID, employee.id, employee.version, hash(csrf), launchHash],
    );
    await tx.query(
      "INSERT INTO audit(org_id,actor_id,action,object_id) VALUES($1,$2,'auth.login',$3)",
      [c.ORG_ID, employee.id, employee.id],
    );
    return {
      token: secret,
      csrf,
      employee,
      organization: { name: c.ORG_NAME, timezone: c.ORG_TIMEZONE },
...
export function capabilities(e: Employee) {
  return {
    support: true,
    act_on_others: e.role !== 'support',
    admin: e.role === 'admin',
    operations: e.role !== 'support',
  };
}
