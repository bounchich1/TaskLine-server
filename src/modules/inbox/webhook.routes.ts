...
export const webhookRoutes: FastifyPluginAsync<{ inbox: Inbox; config: Config }> = async (app, { inbox, config }) => {
    app.post('/webhooks/max', async (request, reply) => {
        const secret = request.headers['x-max-bot-api-secret'];

        ensure(typeof secret === 'string' && equal(secret, config.MAX_WEBHOOK_SECRET), 'webhook_unauthorized', 401);
