...
export async function cancelCycleDeliveries(tx: Sql, cycleId: string): Promise<void> {
    ...
    await tx.query("UPDATE deliveries SET state='canceled' WHERE cycle_id=$1 AND state IN ('queued','retry_wait')", [
        cycleId,
    ]);
}

export async function cancelClientDeliveries(tx: Sql, clientId: string): Promise<void> {
    await tx.query("UPDATE deliveries SET state='canceled' WHERE client_id=$1 AND state IN ('queued','retry_wait')", [
        clientId,
    ]);
