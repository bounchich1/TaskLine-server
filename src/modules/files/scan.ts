...
export async function scanAttachment(deps: FileDeps, id: string): Promise<void> {
    ...
    const file = await one<Attachment>(db, 'SELECT * FROM attachments WHERE org_id=$1 AND id=$2', [ctx.org, id]);
    ...
    try {
        ...
        if (!isAllowedContent(contentType, file.kind)) {
            await db.query("UPDATE attachments SET status='rejected',extraction_status='unsupported' WHERE id=$1", [
                id,
            ]);
