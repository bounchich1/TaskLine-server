...
async function ensureNothingInFlight(tx: Sql, { client, ticket }: { client: Client; ticket: Ticket }): Promise<void> {
    ...
    ensure(!pendingReply, 'delivery_pending', 409, 'Дождитесь доставки ответов или отмените отправку.');

    const pendingInput = await one(tx, "SELECT id FROM inbox WHERE client_id=$1 AND state='pending' LIMIT 1", [
        client.id,
    ]);

    ensure(!pendingInput, 'input_pending', 409, 'Обрабатывается новое сообщение клиента. Повторите закрытие.');
}
