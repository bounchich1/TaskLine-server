...
export async function lockCommandTarget(
    ...
    const ref = await one<{ client_id: string }>(tx, 'SELECT client_id FROM tickets WHERE org_id=$1 AND id=$2', [
        ctx.org,
        ticketId,
    ]);
    ...
    const client = await requireOne<Client>(tx, 'SELECT * FROM clients WHERE org_id=$1 AND id=$2 FOR UPDATE', [
        ctx.org,
        ref.client_id,
    ]);

    const ticket = await requireOne<Ticket>(tx, 'SELECT * FROM tickets WHERE org_id=$1 AND id=$2 FOR UPDATE', [
        ctx.org,
        ticketId,
    ]);
