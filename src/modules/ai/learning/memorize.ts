...
export async function memorize(
    ...
    const confirmed =
        confirmedResolution(resolution, snapshot.entries) && snapshot.missing.length === 0 && !containsSecrets;
...
async function storeResolution(
    ...
    const ticket = await requireOne<Ticket>(tx, 'SELECT * FROM tickets WHERE id=$1', [cycle.ticket_id]);
...
async function insertRecord(
    tx: Sql,
    ctx: Ctx,
    { snapshot, resolution, confirmed }: { snapshot: Snapshot; resolution: Resolution; confirmed: boolean },
    ticket: Ticket,
): Promise<Row> {
    ...
    const record = inserted ?? (await one(tx, 'SELECT * FROM memory_records WHERE source_key=$1', [sourceKey]));
