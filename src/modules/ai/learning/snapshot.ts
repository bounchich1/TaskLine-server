...
async function lockCycle(tx: Sql, ctx: Ctx, job: Job): Promise<{ cycle: Closure; ticket: Ticket }> {
    const initial = await one<Closure>(tx, 'SELECT * FROM closures WHERE id=$1 AND org_id=$2', [job.ref_id, ctx.org]);
    ...
    const cycle = await requireOne<Closure>(tx, 'SELECT * FROM closures WHERE id=$1 FOR UPDATE', [initial.id]);
