...
import { RECORD_JOINS, serializeRecord, type MemoryDeps, type MemoryRecord } from './memory-record.js';
...
async function claimWrite(tx: Sql, { config }: MemoryDeps, id: string): Promise<MemoryRecord | null> {
    const org = config.ORG_ID;

    const row = await one<MemoryRecord>(tx, 'SELECT * FROM memory_records WHERE org_id=$1 AND id=$2 FOR UPDATE', [
        org,
        id,
    ]);
    ...
    if (!live) {
        await tx.query("UPDATE memory_records SET eligible=false,state='invalidated' WHERE id=$1", [id]);
...
async function writeUpstream({ config, upstream }: MemoryDeps, record: MemoryRecord): Promise<UpstreamWrite> {
...
async function recordUpstream({ db }: MemoryDeps, record: MemoryRecord, written: UpstreamWrite): Promise<void> {
...
export async function releaseWriter(db: Sql, org: string, recordId: string): Promise<void> {
    await db.query('UPDATE memory_writer SET holder=NULL WHERE org_id=$1 AND holder=$2', [org, recordId]);
}
