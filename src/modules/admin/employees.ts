...
export async function createEmployee(tx: Sql, org: string, body: EmployeeBody) {
    return one(tx, 'INSERT INTO employees(org_id,max_user_id,name,role,blocked) VALUES($1,$2,$3,$4,$5) RETURNING *', [
        org,
        body.max_user_id,
        body.name,
        body.role,
...
export async function updateEmployee(
    ...
    const old = await one<Employee>(tx, 'SELECT * FROM employees WHERE org_id=$1 AND id=$2 FOR UPDATE', [org, id]);
