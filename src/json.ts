...
export function strictJson(text: string, losslessIds = false, maxBytes = 32768): unknown {
  ...
  const value = (depth: number): unknown => {
    ...
    if (ch === '{') {
      ...
      for (;;) {
        ws(); if (text[at] !== '"') throw new Error('invalid_json_key'); const key = str();
        if (keys.has(key)) throw new Error('duplicate_json_key'); keys.add(key);
        ws(); if (text[at++] !== ':') throw new Error('invalid_json_colon');
        result[key] = value(depth+1); ws();
        const end = text[at++]; if (end === '}') return result; if (end !== ',') throw new Error('invalid_json_object');
      }
    }
    if (ch === '[') {
      at++; ws(); const result: unknown[] = []; if (text[at] === ']') { at++; return result; }
      for (;;) { result.push(value(depth+1)); ws(); const end = text[at++]; if (end === ']') return result; if (end !== ',') throw new Error('invalid_json_array'); }
    }
    const literal = /^(true|false|null|-?(?:0|[1-9]\d*)(?:\.\d+)?(?:[eE][+-]?\d+)?)/.exec(text.slice(at));
    if (!literal) throw new Error('invalid_json_value'); at += literal[0].length;
    const parsed: unknown = JSON.parse(literal[0]);
    if (typeof parsed === 'number' && !Number.isFinite(parsed)) throw new Error('nonfinite_json_number');
    if (losslessIds && typeof parsed === 'number' && /^-?\d+$/.test(literal[0]) && !Number.isSafeInteger(parsed)) return literal[0];
    return parsed;
  };
  const result = value(0); ws(); if (at !== text.length) throw new Error('trailing_json'); return result;
}
...
export function decimalId(value: unknown): string {
  ...
  const id = String(value); if (!/^-?\d{1,20}$/.test(id)) throw new Error('invalid_id'); return id;
}
