...
export function strictJson(text: string, losslessIds = false, maxBytes = 32768): unknown {
  ...
  const value = (depth: number): unknown => {
    ...
    if (ch === '{') {
      ...
      for (;;) {
        ws();
        if (text[at] !== '"') throw new Error('invalid_json_key');
        const key = str();
        if (keys.has(key)) throw new Error('duplicate_json_key');
        keys.add(key);
        ws();
        if (text[at++] !== ':') throw new Error('invalid_json_colon');
    ...
    if (ch === '[') {
      ...
      for (;;) {
        ...
        const end = text[at++];
        if (end === ']') return result;
        if (end !== ',') throw new Error('invalid_json_array');
      }
    }
  ...
  ws();
  if (at !== text.length) throw new Error('trailing_json');
  return result;
}
...
export function decimalId(value: unknown): string {
  ...
  if (!/^-?\d{1,20}$/.test(id)) throw new Error('invalid_id');
  return id;
}
