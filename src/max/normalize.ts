...
export function normalizeUpdate(raw: string): ClientInput {
  ...
  const kind = String(update.update_type);
  ...
  if (kind === 'message_callback') {
    const callback = object(update.callback);
    ...
    return {
      ...
      callbackPayload: String(callback.payload ?? ''),
    };
  }
  ...
  for (const item of Array.isArray(body.attachments) ? body.attachments.slice(0, 11) : []) {
    ...
    attachments.push({
      ...
      filename: String(attachment.filename ?? `${attachment.type}`),
