...
const limitFor = (kind: string) =>
  (kind === 'video' ? 100 : kind === 'image' ? 20 : 25) * 1024 * 1024;
...
export class Files {
  ...
  async read(key: string): Promise<Readable> {
    if (this.s3) {
      const result = await this.s3.send(
        new GetObjectCommand({ Bucket: this.c.S3_BUCKET, Key: key }),
      );
  ...
  async prepare(employee: Employee, ticketId: string, filename: string, kind: string) {
    return this.db.tx(async (tx) => {
      ...
      const row = await one(
        tx,
        'INSERT INTO attachments(org_id,ticket_id,owner_id,filename,kind) VALUES($1,$2,$3,$4,$5) RETURNING id,status',
        [this.c.ORG_ID, ticketId, employee.id, safeFilename(filename), kind],
      );
      return row;
    });
  }
  async receive(
    ...
    const row = await this.db.tx(async (tx) => {
      const file = await one<Attachment>(
        tx,
        "SELECT * FROM attachments WHERE id=$1 AND org_id=$2 AND owner_id=$3 AND message_id IS NULL AND status='uploading' AND expires_at>now() FOR UPDATE",
        [id, this.c.ORG_ID, employee.id],
      );
      ensure(file, 'invalid_upload', 409);
      await tx.query("UPDATE attachments SET status='receiving' WHERE id=$1", [id]);
      return file;
    });
    try {
      await this.storeStream(row, stream, isTruncated);
    } catch (error) {
      await this.db.query("UPDATE attachments SET status='failed' WHERE id=$1", [id]);
      throw error;
    }
  ...
  private async storeStream(
    ...
    const temp = await mkdtemp(join(tmpdir(), 'max-file-'));
  ...
  async downloadInbound(id: string) {
    ...
    if (!consent) {
      await this.db.query("UPDATE attachments SET status='canceled',source_ref=NULL WHERE id=$1", [
        id,
      ]);
      return;
    }
    ...
    if (!source.url) {
      await this.db.query("UPDATE attachments SET status='unavailable' WHERE id=$1", [id]);
      return;
    }
    ...
    try {
      ...
      ensure(!size || size <= limitFor(file.kind), 'file_too_large', 422);
  ...
  async scan(id: string) {
    ...
    try {
      ...
      const allowed =
        (contentType.startsWith('image/') && !['image/svg+xml'].includes(contentType)) ||
      ...
      if (
        !allowed ||
        (file.kind === 'image' && !contentType.startsWith('image/')) ||
        (file.kind === 'video' && !contentType.startsWith('video/'))
      ) {
        await this.db.query(
      ...
      const clean =
        this.c.SCANNER_MODE === 'mock'
          ? !(await readFile(path)).includes('EICAR-STANDARD-ANTIVIRUS-TEST-FILE')
          : await clamScan(path, this.c.CLAMAV_HOST, this.c.CLAMAV_PORT);
      if (!clean) {
        await this.db.query("UPDATE attachments SET status='infected' WHERE id=$1", [id]);
        return;
      }
      const text = contentType === 'text/plain' ? await readFile(path, 'utf8') : null;
      const extracted = text === null ? null : text.slice(0, 32000);
      const coverage = text === null ? 'unsupported' : text.length > 32000 ? 'partial' : 'complete';
      await this.db.tx(async (tx) => {
        ...
        if (!eligible) {
          await tx.query("UPDATE attachments SET status='canceled' WHERE id=$1", [id]);
          return;
        }
        await tx.query(
          "UPDATE attachments SET status='clean',mime=$2,extraction=$3,extraction_status=$4,source_ref=NULL WHERE id=$1",
          [id, contentType, extracted, coverage],
        );
        await emit(tx, this.c.ORG_ID, 'attachment.changed', file.ticket_id, {
          attachment_id: id,
          status: 'clean',
        });
      });
  ...
  async materialize(id: string) {
    const file = await one<Attachment>(
      this.db,
      "SELECT * FROM attachments WHERE org_id=$1 AND id=$2 AND status='clean'",
      [this.c.ORG_ID, id],
    );
    ensure(file?.object_key, 'attachment_not_ready', 422);
    const temp = await mkdtemp(join(tmpdir(), 'max-outbound-'));
    const path = join(temp, 'body');
    try {
      await pipeline(await this.read(file.object_key), createWriteStream(path, { mode: 0o600 }));
    } catch (error) {
...
async function clamScan(path: string, host: string, port: number): Promise<boolean> {
  ...
  const result = new Promise<boolean>((resolve, reject) => {
    let response = '';
    socket.setTimeout(60000, () => socket.destroy(new Error('scanner_timeout')));
...
export async function writeTestText(path: string, content: string) {
