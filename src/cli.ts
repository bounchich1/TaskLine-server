...
try {
  ...
  if (command === 'migrate') {
    await migrate(db);
    await seed(db, c);
    console.log('Schema and reserved defaults ready.');
    ...
    await db.tx(async (tx) => {
      ...
      const existing = await one(
        tx,
        "SELECT id FROM employees WHERE org_id=$1 AND role='admin' AND NOT blocked",
        [c.ORG_ID],
      );
      ensure(
        !existing,
        ...
        409,
        'Use authenticated administration after initial bootstrap.',
      );
      const id = randomUUID();
      await tx.query(
        "INSERT INTO employees(id,org_id,max_user_id,name,role) VALUES($1,$2,$3,$4,'admin')",
        [id, c.ORG_ID, c.BOOTSTRAP_MAX_USER_ID, c.BOOTSTRAP_NAME],
      );
    ...
    console.log('Initial administrator created.');
      ...
      c.MAX_MODE === 'live' && new URL(c.PUBLIC_URL).protocol === 'https:',
      'https_live_required',
      422,
    );
    await new MaxClient(c).request('/subscriptions', {
      url: `${c.PUBLIC_URL}/webhooks/max`,
  ...
  } else if (command === 'memory-reconcile') {
    const id = process.argv[3];
    ensure(id, 'record_id_required', 422);
    await new Memory(db, c).reconcile(id);
    console.log('Memory reconciliation completed.');
  } else if (command === 'permit-resolve') {
    const slot = Number(process.argv[3]);
    const evidence = process.argv[4];
    ensure(
      Number.isInteger(slot) && slot >= 1 && slot <= 15 && evidence?.length >= 20,
    ...
    await db.tx(async (tx) => {
      ...
      await audit(tx, c.ORG_ID, null, 'ai.permit.resolved', String(slot), {
        evidence,
        holder: permit.holder,
      });
      await tx.query(
        "UPDATE ai_calls SET state='failed',reason='operator_confirmed_ended',finished_at=now() WHERE id=$1",
        [permit.holder],
      );
      await tx.query(
        "UPDATE ai_permits SET state='free',holder=NULL,started_at=NULL WHERE slot=$1",
        [slot],
      );
    });
    ...
    await db.tx(async (tx) => {
      ...
      const busy = await one(tx, "SELECT count(*)::int AS n FROM ai_permits WHERE state<>'free'");
