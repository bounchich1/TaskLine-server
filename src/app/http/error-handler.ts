...
export function handleError(error: HandlerError, request: FastifyRequest, reply: FastifyReply): FastifyReply {
    const known = describeKnownError(error);

    if (!known) {
        request.log.error({ reason: error.code ?? error.name, request_id: request.id }, 'Request failed');
    }
