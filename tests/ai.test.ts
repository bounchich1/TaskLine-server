...
class FakeMemory implements MemoryTransport {
  ...
  remember(content: string, project: string) {
    ...
    return this.unknown
      ? Promise.reject(new Error('timeout_after_commit'))
...
it('applies triage once without overwriting a manual field', async () => {
  ...
  const workflow = new Workflows(
    context.db,
    context.c,
    new Gateway(context.db, context.c),
    noRecall,
  );
...
it('retains uncertain permits and refuses duplicate provider dispatch', async () => {
  ...
  expect(calls).toBe(1);
  expect(
    (await one(context.db, "SELECT count(*)::int AS n FROM ai_permits WHERE state='uncertain'"))!.n,
  ).toBe(1);
});
it('requires a real memory tool call, verifies persistence and removes reopened cases from recall', async () => {
  ...
  await context.db.query(
    "UPDATE messages SET delivery_state='delivered' WHERE author_type='staff'",
  );
...
it('rejects invented memorization prose without a tool receipt', async () => {
  ...
  await expect(
    new Workflows(context.db, context.c, model, noRecall).learning(job),
