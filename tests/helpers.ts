...
function domainAdapter(db: Database, config: Config) {
    ...
    return {
        ...
        command: (...[actor, ticketId, name, body, expectedVersion, idempotencyKey]: LegacyCommandArgs) =>
...
export async function fixture(existing?: Database, config = testConfig()) {
    ...
    const input = async (text: string, userId = '100') => {
        ...
        const client = await requireOne<Client>(db, 'SELECT * FROM clients WHERE org_id=$1 AND max_user_id=$2', [
            config.ORG_ID,
            userId,
        ]);
    ...
    const command = async (name: string, body: Record<string, unknown> = {}, actor = staff, userId = '100') => {
