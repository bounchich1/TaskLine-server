...
describe('consent-first routing', () => {
    it('buffers encrypted content, creates no staff/AI data, then promotes in order', async () => {
        ...
        expect(
            (await context.db.query("SELECT text FROM messages WHERE author_type='client' ORDER BY seq")).rows.map(
                (row) => row.text,
            ),
        ).toEqual(['Мой секрет до согласия', 'Второе сообщение']);

        expect((await one(context.db, "SELECT count(*)::int AS n FROM jobs WHERE kind='triage'"))!.n).toBe(1);
    });
    ...
    it('stale policy buttons do not grant new policy; decline removes buffers', async () => {
        ...
        await context.db.query("UPDATE callback_actions SET policy_version='old' WHERE client_id=$1", [client.id]);
        await context.accept(client);
    ...
    it('withdrawal closes slot and invalidates learning without a rating', async () => {
        ...
        expect((await one(context.db, "SELECT state FROM jobs WHERE kind='learning'"))!.state).toBe('canceled');
        expect((await one(context.db, 'SELECT count(*)::int AS n FROM deletion_tombstones'))!.n).toBe(1);
    });
});
...
describe('staff transitions and rating cycles', () => {
    it('replays commands and rejects same key with different payload', async () => {
        ...
        const first = await context.domain.command(context.staff, ticket.id, 'assign', {}, ticket.version, key);
        const second = await context.domain.command(context.staff, ticket.id, 'assign', {}, ticket.version, key);

        expect(second.id).toBe(first.id);

        await expect(
            context.domain.command(context.staff, ticket.id, 'assign', { other: true }, ticket.version, key),
    ...
    it('persists closure, rating, reopen history and one learning obligation per cycle', async () => {
        ...
        expect((await one(context.db, "SELECT count(*)::int AS n FROM jobs WHERE kind='learning'"))!.n).toBe(2);
        expect((await one(context.db, 'SELECT rating FROM closures WHERE cycle_no=1'))!.rating).toBe(10);
    });
    ...
    it('a different slot prevents reopening an older ticket', async () => {
        ...
        const version = Number((await one(context.db, 'SELECT version FROM tickets WHERE id=$1', [old.id]))!.version);

        await expect(
            context.domain.command(context.staff, old.id, 'reopen', { reason: 'test' }, version, randomUUID()),
    ...
    it('expiry suppresses obsolete reminder and respects already-received rating', async () => {
        ...
        expect(
            (await one(context.db, "SELECT count(*)::int AS n FROM deliveries WHERE kind='rating_reminder'"))!.n,
        ).toBe(0);
    });
