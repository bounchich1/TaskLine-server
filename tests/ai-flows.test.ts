...
function validTriage(job: Job): Row {
  ...
  const code = (dimension: string) =>
    dictionaries.find((entry) => entry.dimension === dimension)?.code;
  return {
    schema_version: '1.0',
...
it('runs triage through both recall tools and one schema repair', async () => {
  ...
  expect(steps.rows.map((row) => row.step_key)).toEqual([
    'triage-0',
    'triage-1',
    'triage-2',
    'triage-repair',
...
it('summarizes a long conversation in chunks, reduces the summaries, then memorizes', async () => {
  ...
  const keys = await context.db.query('SELECT step_key FROM ai_checkpoints WHERE job_id=$1', [
    job.id,
  ]);
