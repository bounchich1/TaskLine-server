...
it('defers a triage job while the model is busy, without counting a retry', async () => {
    ...
    const stored = await one(context.db, 'SELECT state,reason,payload FROM jobs WHERE id=$1', [job.id]);
...
it('gives up on learning after six failed attempts', async () => {
    ...
    await context.db.query(`UPDATE jobs SET payload=jsonb_set(payload,'{retry_count}','5') WHERE id=$1`, [job.id]);
...
it('expires overdue triage and recovers abandoned work during maintenance', async () => {
    ...
    await context.db.query("UPDATE ai_permits SET state='running',started_at=now()-interval '4 minutes' WHERE slot=1");
    ...
    const triage = await one(context.db, "SELECT state FROM jobs WHERE kind='triage' AND ref_id=$1", [ticket.id]);
