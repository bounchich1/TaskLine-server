...
it('serves the admin console to admins and operations to non-support staff', async () => {
    ...
    const [template] = templates.json<{ items: { code: string; body: string; version: number }[] }>().items;
...
it('feeds dev inbound messages to the inbox for operations staff only', async () => {
    ...
    const inbound = await one(context.db, 'SELECT count(*)::int AS n FROM inbox WHERE org_id=$1', [context.c.ORG_ID]);
