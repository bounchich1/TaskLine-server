...
it('lists employees, dictionaries, counts and notifications, and marks one read', async () => {
    ...
    const stored = await one(context.db, 'SELECT read_at FROM notifications WHERE id=$1', [notification.id]);
...
it('starts the SSE stream from now when no cursor is given', async () => {
    await context.create();
    const before = await one<{ cursor: string }>(context.db, 'SELECT max(cursor)::text AS cursor FROM ui_events');
